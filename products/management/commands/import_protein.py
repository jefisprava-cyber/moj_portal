...
class Command(BaseCommand):
    help = 'Import Protein.sk (Robust Logic)'

    def handle(self, *args, **kwargs):
        # 1. NASTAVENIA
        url = "https://www.protein.sk/feed/heureka.xml"
        SHOP_NAME = "Protein.sk"
        ...
        default_cat, _ = Category.objects.get_or_create(slug='sportova-vyziva', defaults={'name': 'Športová výživa'})
        ...
        try:
            ...
            for event, elem in context:
                ...
                if tag not in ['shopitem', 'item', 'entry']: continue
                ...
                try:
                    ...
                    if not name or not price_str or not raw_url: elem.clear(); continue
                    ...
                    product = None
                    if ean and len(ean) > 6: product = Product.objects.filter(ean=ean).first()
                    if not product: product = Product.objects.filter(name=name).first()
                    ...
                    if product:
                        ...
                        product = Product.objects.create(
                            name=name, slug=unique_slug, description=description,
                            price=price, category=category, image_url=image_url, ean=ean
                        )
                        created_count += 1
                    ...
                    errors += 1
                    if errors == 1: self.stdout.write(self.style.WARNING(f"⚠️ Chyba pri '{name}': {e}"))
                finally:
                    elem.clear()
